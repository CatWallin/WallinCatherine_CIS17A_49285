...
int  srch1( char [], char [],int);//Search for 1 occurrence
...
int main(int argc, char** argv) {
    ...
    int sentenceLength = sizeof(sntnce)/sizeof(sntnce[0]);

    //Search for the pattern
    //https://slack-imgs.com/?c=1&o1=gu&url=https%3A%2F%2Fa.slack-edge.com%2Fproduction-standard-emoji-assets%2F10.2%2Fgoogle-small%2F2615.pngInput the sentence and pattern, Output the matching positions
    //Remember, indexing starts at 0 for arrays.
    int c = srch1(sntnce, pattern, LINE);
...
int  srch1( char sntnce[], char pattern[],int LINE){
    for (int i = 0; i < LINE; i++){
        if (sntnce[i] == pattern[i]){
            return i;
        }
        return -1;
    }
...
void srchAll( char [], char [],int []){
    
    return;
}
void print(char sntnce[]){
