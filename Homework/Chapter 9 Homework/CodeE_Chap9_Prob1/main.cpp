 ...
 * Created on September 28, 2020 7:45 PM
 */
...
int *mode(const int *array,int arySize){
    ...
    int frequency = 1;
    int modeTotal = 0;
    int currentValue = ary[0];
    
    for (int i = 1; i < arySize; i++){
        if (ary[i] == currentValue){
            frequency++;
    ...
    int nmodes=modeTotal;
    int *modeAry=new int[nmodes+2];
    modeAry[0]=nmodes;  //Stub returns no modes
    modeAry[1]=maxFrequency;//Stub returns Frequency 1
    
    //Fill the mode array
    currentValue = ary[0];
    frequency = 1;
    int modeArrayIndex = 2;
    for (int i = 1; i < arySize; i++){
        if (ary[i] == currentValue){
            frequency++;
        }
            ...
            if (frequency == maxFrequency){
                modeAry[modeArrayIndex] = currentValue;
                modeArrayIndex++;
            }
            frequency = 1;
            currentValue = ary[i];
